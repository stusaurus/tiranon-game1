...
function resetPlayerPosition(){playerX=(playArea.clientWidth-player.offsetWidth)/2;player.classList.remove("facing-left");drawPlayer();}
function drawPlayer(){player.style.left=`${playerX}px`;player.style.transform="none";}
function setFacing(direction){player.classList.toggle("facing-left",direction==="left");}
...
function gameLoop(currentTime){if(!isPlaying)return;const dt=Math.min((currentTime-previousTime)/1000,.05);previousTime=currentTime;if(movingLeft){playerX-=PLAYER_SPEED*dt;setFacing("left");}if(movingRight){playerX+=PLAYER_SPEED*dt;setFacing("right");}const maximumX=playArea.clientWidth-player.offsetWidth;playerX=Math.max(0,Math.min(playerX,maximumX));drawPlayer();updateStars(dt);animationId=requestAnimationFrame(gameLoop);}
function endGame(){isPlaying=false;movingLeft=false;movingRight=false;cancelAnimationFrame(animationId);clearInterval(timerId);clearInterval(starTimerId);finalScoreDisplay.textContent=`最終スコア ${score}`;gameOverScreen.hidden=false;}
...
window.addEventListener("keydown",event=>{if(["ArrowLeft","ArrowRight","KeyA","KeyD"].includes(event.code))event.preventDefault();if(event.code==="ArrowLeft"||event.code==="KeyA"){movingLeft=true;setFacing("left");}if(event.code==="ArrowRight"||event.code==="KeyD"){movingRight=true;setFacing("right");}});
window.addEventListener("keyup",event=>{if(event.code==="ArrowLeft"||event.code==="KeyA")movingLeft=false;if(event.code==="ArrowRight"||event.code==="KeyD")movingRight=false;});
window.addEventListener("blur",()=>{movingLeft=false;movingRight=false;});
function addHoldControls(button,direction){button.addEventListener("pointerdown",event=>{event.preventDefault();button.setPointerCapture(event.pointerId);if(direction==="left")movingLeft=true;if(direction==="right")movingRight=true;setFacing(direction);});const stop=()=>{if(direction==="left")movingLeft=false;if(direction==="right")movingRight=false;};button.addEventListener("pointerup",stop);button.addEventListener("pointercancel",stop);button.addEventListener("lostpointercapture",stop);}
